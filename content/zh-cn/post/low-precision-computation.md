...
```
数据层：  [v₁ v₂ ... v₁₆]  ← 每 16 个 E2M1 值为一组
          ↕
Block scale：  s_block (E4M3, 8-bit)  ← 每组 1 个
          ↕
Tensor scale：  s_tensor (FP32)  ← 整个 tensor 1 个
```

**MXFP4（OCP 开放标准）**：

```
数据层：  [v₁ v₂ ... v₃₂]  ← 每 32 个 E2M1 值为一组
          ↕
Block scale：  s_block (E8M0, 8-bit, power-of-2)  ← 每组 1 个
```
...
另外，"4.25 bits"这个数字也值得留意——FP4 表面上是 4-bit，但加上 scale 的存储开销后，每个参数实际占 4.25-4.5 bits。4-bit 不是字节对齐的，存储时需要将两个 FP4 值 pack 到一个 byte 中，scale 也要紧凑排列：
...
这给 kernel 实现和内存访问模式带来了额外复杂度——读取单个 FP4 值需要位移和掩码操作，scale 的对齐也需要专门处理。
...
```
训练时（模拟量化）：
FP32 master weights → 量化到 FP4 → 无损反量化到 FP8 → 前向计算
反向传播：梯度直接从 FP8 权重传回 FP32 master weights（Straight-Through Estimator, STE）

推理时（真实量化）：
直接使用原生 FP4 权重 → 实际加速 + 显存节省
```
