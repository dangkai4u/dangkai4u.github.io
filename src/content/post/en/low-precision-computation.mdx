...
Also worth noting is the "4.25 bits" figure — FP4 is nominally 4-bit, but with scale storage overhead, each parameter effectively costs 4.25-4.5 bits. Moreover, 4-bit is not byte-aligned, so two FP4 values must be packed into a single byte, with scales also tightly arranged. This adds complexity to kernel implementation and memory access patterns — reading a single FP4 value requires bit shifting and masking, and scale alignment needs special handling.
