...
**DeepSeek-V3** uses custom scaling: block-wise 128×128 for weights and tile-wise 1×128 for activations, with scales stored in FP32. Blackwell had not been released at the time of training, so MXFP8 could only run via software emulation on Hopper without native hardware acceleration. FP32 scales also offer better precision than E8M0's power-of-2 constraint. The result: FP8 training loss within 0.25% of the BF16 baseline [1].
