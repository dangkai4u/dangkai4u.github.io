...
**DeepSeek-V3's choice** did not use MXFP8, opting instead for custom scaling: block-wise 128×128 for weights (weight distributions are stable, block granularity suffices) and tile-wise 1×128 for activations (activations have more outliers, requiring finer granularity). Scales are stored in FP32. One key reason is that Blackwell had not yet been released when DeepSeek-V3 was trained — MXFP8 on Hopper is software-emulated without native hardware acceleration, offering limited throughput gains. Custom scaling with FP32 scales provided better precision and engineering controllability. The result is FP8 training loss within 0.25% of the BF16 baseline [1].
