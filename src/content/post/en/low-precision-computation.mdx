...
**DeepSeek-V3's choice** did not use MXFP8, opting instead for custom scaling: block-wise 128×128 for weights (weight distributions are stable, block granularity suffices) and tile-wise 1×128 for activations (activations have more outliers, requiring finer granularity). Scales are stored in FP32. This approach offers greater flexibility but requires more engineering effort. The result is FP8 training loss within 0.25% of the BF16 baseline [1].
