...
NVFP4 uses smaller blocks (16 vs 32) and two-level scaling for better precision — in GPT-OSS fine-tuning experiments, NVFP4's validation loss was about 2-3% lower than MXFP4 [4]. However, MXFP4's E8M0 power-of-2 scale only requires bit-shift for dequantization, making it simpler in hardware with slightly better throughput; it also enjoys broader hardware support as an OCP open standard. In practice, the choice depends on precision vs ecosystem priorities. Note the "4.25 bits" figure: FP4 is nominally 4-bit, but with scale storage overhead, each parameter effectively costs 4.25-4.5 bits.
