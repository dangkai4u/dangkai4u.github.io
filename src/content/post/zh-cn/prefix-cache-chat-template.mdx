...
截断不可避免，但补闭合的 token 破坏 prefix。Renderers <Ref n={8} /> 的 bridge 在拼接点合成 close token——只追加不重写——是正确路径。注意 content 截断（无 stop token）不影响前缀匹配：stop token 追加在模型已生成 token 的末尾，不改写已有 token，prefix cache 可匹配到模型最后一个生成 token 为止。
...
<span id="ref-1">[1]</span> vLLM. Automatic Prefix Caching. [链接](https://docs.vllm.ai/en/latest/design/automatic_prefix_caching.html)

<span id="ref-2">[2]</span> Zheng, L., et al. SGLang: Efficient Execution of Structured Language Model Programs. arXiv:2312.07104. [链接](https://arxiv.org/abs/2312.07104)

<span id="ref-3">[3]</span> OpenAI. Prompt Caching Guide & Conversation State. [Prompt Caching](https://developers.openai.com/api/docs/guides/prompt-caching) | [Conversation State](https://developers.openai.com/api/docs/guides/conversation-state) | [Prompt Caching 201](https://developers.openai.com/cookbook/examples/prompt_caching_201)

<span id="ref-4">[4]</span> OpenAI. Harmony Response Format. [链接](https://github.com/openai/harmony)

<span id="ref-5">[5]</span> Anthropic. Prompt Caching. [链接](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching)

<span id="ref-6">[6]</span> Anthropic. Extended Thinking. [链接](https://docs.anthropic.com/en/docs/build-with-claude/extended-thinking)

<span id="ref-7">[7]</span> DeepSeek. DeepSeek-V4-Pro Encoding Module. [链接](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/encoding/encoding_dsv4.py)

<span id="ref-8">[8]</span> PrimeIntellect. Renderers: Prefix-Preserving Chat Template Rendering. [链接](https://github.com/PrimeIntellect-ai/renderers)
