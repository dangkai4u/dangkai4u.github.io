...
还有一个容易混淆的概念：**FP8 和 MXFP8**。上面讨论的 FP8（E4M3 / E5M2）是"裸格式"——每个值独立存储为 8-bit 浮点数，scaling 的粒度、格式、存储方式全部由框架自行决定。DeepSeek-V3 的 block-wise / tile-wise scaling 就是这种模式。

**MXFP8** 是 OCP（Open Compute Project）提出的 Microscaling 标准，属于 MX 系列。它把 block-level scaling 直接内建到了格式规范中：每 32 个元素共享一个 E8M0 格式的 scale（8 位纯指数，无尾数，只能表示 2 的幂次）。反量化只需位移操作，硬件实现简单；但每参数实际开销为 8.25 bits（8 + 8/32），且 block 大小固定为 32，灵活性不如自定义 scaling。

简单说：标准 FP8 把 scaling 留给用户（灵活但需要工程投入），MXFP8 把 scaling 标准化到格式中（简化互操作但粒度固定）。后面 3.3 节讨论的 MXFP4 属于同一个 MX 系列。
...
NVFP4 用更小的块（16 vs 32）和两层缩放换取了更好的精度，在 GPT-OSS 微调实验中 validation loss 比 MXFP4 低约 2-3% [4]。

但 MXFP4 的 E8M0 power-of-2 scale 反量化只需位移操作，硬件实现更简单，吞吐略优；同时作为 OCP 开放标准获得了更广泛的硬件支持。实际选择取决于项目对精度和生态的优先级。

另外，"4.25 bits"这个数字也值得留意——FP4 表面上是 4-bit，但加上 scale 的存储开销后，每个参数实际占 4.25-4.5 bits。

而且 4-bit 不是字节对齐的，存储时需要将两个 FP4 值 pack 到一个 byte 中，scale 也要紧凑排列。读取单个 FP4 值需要位移和掩码操作，scale 的对齐也需要专门处理，这给 kernel 实现带来了额外复杂度。
...
DeepSeek-V4 对 MoE Expert 权重采用 FP4（MXFP4）QAT，其中有一个关键设计：**FP4→FP8 的反量化是无损的** [2]。FP8 E4M3 比 FP4 E2M1 多 2 位指数，动态范围更大。只要同一 FP8 量化块内各 FP4 子块的 scale 比值不超过一定阈值，FP4 的细粒度 scale 信息可以被 FP8 的动态范围完全吸收。

这意味着整个 QAT 流程可以直接复用现有的 FP8 训练框架，无需修改反向传播逻辑。
