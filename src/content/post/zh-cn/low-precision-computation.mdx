...
**DeepSeek-V3 的选择** 没有使用 MXFP8，而是自定义 scaling：权重用 block-wise 128×128（权重分布相对稳定，block 粒度足够），激活值用 tile-wise 1×128（激活值 outlier 多，需要更细粒度），scale 均以 FP32 存储。一个关键原因是 DeepSeek-V3 训练时 Blackwell 尚未发布，MXFP8 在 Hopper 上只能软件模拟，缺少原生硬件加速，吞吐优势有限。自定义 scaling 配合 FP32 scale 精度更高、工程可控性更强。最终 FP8 训练的 loss 与 BF16 基线偏差控制在 0.25% 以内 [1]。
