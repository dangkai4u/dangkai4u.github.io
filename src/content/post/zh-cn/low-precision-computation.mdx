...
**DeepSeek-V3** 采用自定义 scaling：权重 block-wise 128×128，激活值 tile-wise 1×128，scale 以 FP32 存储。训练时 Blackwell 尚未发布，MXFP8 在 Hopper 上只能软件模拟，无法获得原生硬件加速；而 FP32 scale 精度优于 E8M0 的 power-of-2 约束，工程可控性也更强。最终 FP8 训练 loss 与 BF16 基线偏差控制在 0.25% 以内 [1]。
