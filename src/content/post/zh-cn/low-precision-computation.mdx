...
图中 1.5 在 FP8 E4M3 中编码为 `0 0111 100`。<Sidenote label="?">**浮点编码**：浮点数存储为三个字段 `[S | E | M]`，还原公式为 (−1)^S × 2^(E−bias) × (1 + M/2^m)，其中 bias = 2^(e−1) − 1，e 为指数位数，m 为尾数位数。FP8 E4M3 中 bias = 2³ − 1 = 7，m = 3。以 `0 | 0111 | 100` 为例：S=0，E=7，M=4，还原为 (−1)⁰ × 2^(7−7) × (1 + 4/8) = 1 × 1 × 1.5 = **1.5**。乘以 2² 后指数 +2：E=9=`1001`，尾数不变 `100`，还原为 2^(9−7) × 1.5 = 4 × 1.5 = **6.0**。</Sidenote>
