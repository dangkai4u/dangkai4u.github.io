...
**DeepSeek-V3 的选择** 没有使用 MXFP8，而是自定义 scaling：权重用 block-wise 128×128（权重分布相对稳定，block 粒度足够），激活值用 tile-wise 1×128（激活值 outlier 多，需要更细粒度），scale 均以 FP32 存储。这种方案灵活性更高但需要更多工程投入。最终 FP8 训练的 loss 与 BF16 基线偏差控制在 0.25% 以内 [1]。
